...
# if your asset is capable of handling more than one access concurrently then use a semaphore
sem = threading.Semaphore(2) # Semaphore can indicate the maximum number of concurrent threads that will work
...
class TestRunner(threading.Thread): # NB here there will be no actual tests
    ...
    def __init__(self, i):
        ...
        self.sem = sem # we grab the global lock (we do not yet use it)
        self.i = i
    ...
    def run(self):
        ...
        while running:
            self.randomDelay()
            self.sem.acquire() # we now have exclusive access to data, we acquired the global lock
            if testsAvailable <=0:
            ...
            self.sem.release() # make the lock available for other threads
